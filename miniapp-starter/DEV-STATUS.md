...
### 6. 跨端云同步 — **已接入**
- `utils/cloud-sync.js` + 云数据库 `user_state` 集合
- 单设备 claim 模型:云端 doc 持有 `sessionId`,谁的 ID 匹配谁能写
- `app.onLaunch` 异步 hydrate;每个 tab 页 `onShow` 调 `hydrateIfStale()`(30s 防抖)
- 切换设备时弹 modal「切到此设备 / 只读浏览」,只读模式 `updateState` 直接拒写
- profile 页有数据同步卡片(状态 + 立即同步 + 切回此设备)
- 同步白名单 `SYNC_FIELDS` = notebooks / tasks / coins / streakDays / bonusCoins / pet / lastReward(OCR 任务、UI 临时态、固定配置不同步)

### 7. 大规模数据性能 — **核心热点已优化**
针对 1000 个作业本 / 5000+ 个作业的目标场景:
- 内存缓存(`_stateCache`)消除每次 onShow 的 storage 读 + JSON 反序列化
...
### 2. 账号体系仍是单一用户
跨端同步是基于「同一个微信账号下不同设备」的 sessionId claim 模型,不区分家长/孩子身份。这意味着:
- 多孩子家庭只能共用一份数据
- 没有"家长查看 / 孩子操作"的权限分层
- 没有家庭成员邀请/关联机制

未来如果要做家庭体系,需要在 `user_state` 之上加 `families / children` 关系。

### 3. 同步粒度较粗
当前 push 是把整段 `SYNC_FIELDS` JSON 推上去,不是 field-level patch。在线状态下没问题(防抖 200ms,基本不冲突),但:
- 长时间离线后回到线上,本地 vs 云端是 last-write-wins
- 单个 doc 体积随作业数线性增长,极端情况可能撞云数据库 single-doc 体积上限

### 4. 线上级异常处理仍不足
- OCR 失败的精细化错误提示(目前都弹"识别失败" modal,没区分 errorCode)
- OCR 重试机制
- 图片上传失败分类处理
- 导入后的任务去重与清洗
...
- **腾讯云 OCR 真实识别返回**
- 草稿编辑 + 批量导入
- **业务数据云同步**(`user_state` 集合,单设备 claim)
- **跨设备 hydrate / 切换设备 modal / 只读模式**
...
`miniapp-starter` 现在是一个 OCR 链路真实可用、跨端云同步落地、核心热点性能已优化的可灰度 MVP;下一步的关键是 **OCR 识别质量调优**、**计费方案评估** 和 **多孩子账号体系**,让它从"灰度可用"走向"正式上线"。
