// 专注计时页 pkg-notebook/task-focus
module.exports = {
  en: {
    tfocus_navtitle: 'In Progress',
    tfocus_est_chip: 'Est. {n} min',
    tfocus_clock_label: 'Time Elapsed',
    tfocus_pause: 'Pause',
    tfocus_resume: 'Resume',
    tfocus_paused_label: 'Paused for',
    tfocus_finish: 'Done',
    tfocus_pomo_min: '{n} min',
    tfocus_pomo_label: '🍅 Break reminder · every {n} min',
    tfocus_remind_rest: 'Focused {n} min — take a short break! 🧘',
    tfocus_remind_resume: 'Break’s over — back to your homework! ✏️',
    tfocus_break_hint: 'Suggested break: {n} min',
    tfocus_pomo_left_pre: '🍅 Break in',
    tfocus_pomo_done: '🍅 Time for a break!',
    tfocus_pomo_stop: '🔕 Stop',
    tfocus_ev_start: 'Started',
    tfocus_ev_pause: 'Paused',
    tfocus_ev_resume: 'Resumed',
    tfocus_ev_done: 'Finished',
    tfocus_tl_title: '⏱ Time log',
    tfocus_tl_empty: 'No time records yet.',
    tfocus_tl_close: 'Close',
    tfocus_tl_view: '⏱ View time log',
    tfocus_next_title: 'Done! 🎉',
    tfocus_next_q: 'Start the next homework right away?',
    tfocus_next_content: 'Start the next homework right away?\n{name}',
    tfocus_next_confirm: 'Start it',
    tfocus_next_cancel: 'Not now',
    tfocus_set_title: '🍅 Pomodoro settings',
    tfocus_set_work: 'Focus session',
    tfocus_set_short: 'Short break',
    tfocus_set_long: 'Long break',
    tfocus_set_thresh: 'Hard-work threshold',
    tfocus_set_window: 'Look-back window',
    tfocus_set_hint: 'When you pause, if your work time within the look-back window is over the threshold, a long break is suggested; otherwise a short one.',
    tfocus_set_close: 'Done',
    tfocus_toast_not_found: 'Task not found'
  },
  zh: {
    tfocus_navtitle: '进行中',
    tfocus_est_chip: '预计 {n} 分钟',
    tfocus_clock_label: '已用时间',
    tfocus_pause: '暂停',
    tfocus_resume: '继续',
    tfocus_paused_label: '已暂停',
    tfocus_finish: '完成',
    tfocus_pomo_min: '{n} 分钟',
    tfocus_pomo_label: '🍅 番茄钟 · 每 {n} 分钟提醒',
    tfocus_remind_rest: '已专注 {n} 分钟,休息一下吧!🧘',
    tfocus_remind_resume: '休息够啦,继续写作业吧!✏️',
    tfocus_break_hint: '建议休息 {n} 分钟',
    tfocus_pomo_left_pre: '🍅 距休息',
    tfocus_pomo_done: '🍅 该休息啦!',
    tfocus_pomo_stop: '🔕 停止',
    tfocus_ev_start: '开始',
    tfocus_ev_pause: '暂停',
    tfocus_ev_resume: '继续',
    tfocus_ev_done: '完成',
    tfocus_tl_title: '⏱ 时间记录',
    tfocus_tl_empty: '还没有时间记录。',
    tfocus_tl_close: '关闭',
    tfocus_tl_view: '⏱ 查看时间记录',
    tfocus_next_title: '完成啦!🎉',
    tfocus_next_q: '直接开始下一项作业?',
    tfocus_next_content: '直接开始下一项作业?\n{name}',
    tfocus_next_confirm: '开始',
    tfocus_next_cancel: '先不用',
    tfocus_set_title: '🍅 番茄钟设置',
    tfocus_set_work: '专注时长',
    tfocus_set_short: '短休息',
    tfocus_set_long: '长休息',
    tfocus_set_thresh: '努力阈值',
    tfocus_set_window: '统计时段',
    tfocus_set_hint: '暂停时,如果「统计时段」内的作业时长超过「努力阈值」,就建议长休息,否则短休息。',
    tfocus_set_close: '完成',
    tfocus_toast_not_found: '作业不存在'
  }
}
