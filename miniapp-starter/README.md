...
当前项目已经走过原型阶段，进入「可灰度 MVP」：OCR 真实闭环已通、跨端云同步落地、核心热点路径已为 1000 本规模做过算法优化。适合做家长访谈和小范围灰度验证。
...
### 已有页面
- `pages/home`：首页，按选定日期展示作业行（未完成 / 已完成 分组）
- `pages/tasks`：作业本列表（按结束日期倒序）
- `pages/notebook-detail`：单作业本结构管理（作业按学科分组）
- `pages/calendar`：月历视图，每日完成度概览
- `pages/pet`：宠物养成页
- `pages/profile`：个人页（含数据同步卡片）
- `pages/stats`：统计页
- `pages/ocr-import`：拍照/选图上传页
- `pages/ocr-result`：OCR 识别结果确认页
- `pkg-notebook/notebook-edit`：作业本编辑（分包，preloadRule 预热）

### 已完成能力
- 多页面交互闭环（首页 / 作业本 / 详情 / 日历 / 宠物 / 统计）
- 一次性 + 重复（每日 / 每周指定日）两种作业本，重复型按日期分别记录状态
- 作业状态流转：未开始 / 进行中 / 已完成 / 误点恢复
...
- **跨端云同步**：`user_state` 集合，单设备 claim 模型（切换设备弹 modal，旧设备只读）
...
### 小程序端
- `app.js / app.json / app.wxss`：全局配置（含 `tabBar.custom` 指向自定义 tab bar、`preloadRule` 预热分包）
- `pages/*`：主包页面
- `pkg-notebook/notebook-edit`：作业本编辑分包
- `components/task-list`：首页和日历共用的作业行组件
- `custom-tab-bar`：自定义底部导航
- `utils/store.js`：本地状态 + 内存缓存 + schema 迁移
...
- `cloudfunctions/homeworkOCR`
  - 多 provider 兜底 OCR
  - 已部署到云开发环境 `cloud1-d8gkzu6ls85efd509`，真实可调用
  - 详见 `CLOUD-SETUP.md`
...
6. 数据云同步需要在云开发控制台手动新建 `user_state` 集合（详见 `CLOUD-SETUP.md`）
...
这是一个 OCR 真实可用、跨端云同步落地、核心热点路径已为大数据规模优化的微信小程序，可以进入小范围灰度验证阶段。

下一步关键是 OCR 识别质量调优 + 多账号体系 + 计费方案评估。
