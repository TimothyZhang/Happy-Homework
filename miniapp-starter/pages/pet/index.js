...
Page({
  ...
  _initActor() {
    ...
    this.setData({
      actorX: 25, actorY: y,
      actorScale: depthForY(y) * (this.data.isLandscape ? LANDSCAPE_SCALE : 1), actorZ: zForY(y),
  ...
  _scheduleWander(delay) {
    ...
    this._wanderTimer = setTimeout(() => {
      ...
      const yMax = this.data.isLandscape ? 88 : FLOOR.yMax   // 横屏:走到书桌下方(~68%)更靠下,但收在 88 避开底部 tab 被挡
  ...
  _moveActorTo(tx, ty, after) {
    ...
    ty = clampNum(ty, FLOOR.yMin, this.data.isLandscape ? 88 : FLOOR.yMax)   // 横屏下界 88(避开底部 tab)
    ...
    this.setData({
      ...
      actorScale: depthForY(ty) * (this.data.isLandscape ? LANDSCAPE_SCALE : 1),
      actorZ: zForY(ty),
